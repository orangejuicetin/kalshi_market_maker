...
class ExchangeClient(KalshiClient):
    ...
    def get_market(self, ticker: str):
        dictr = self.get(self.markets_url + "/" + ticker)
        return dictr
    ...
    def get_market_history(
        ...
        query_string = self.query_generation(params=relevant_params)
        dictr = self.get(self.markets_url + "/" + ticker + "/history" + query_string)
        return dictr
    ...
    def get_orderbook(
        ...
        relevant_params = {k: v for k, v in locals().items() if k != "ticker"}
        query_string = self.query_generation(params=relevant_params)
        dictr = self.get(self.markets_url + "/" + ticker + "/orderbook" + query_string)
        return dictr
